...
Block::Block()
: Block(SpriteManager::current()->create(get_default_sprite_name()))
{
};

Block::Block(const ReaderMapping& lisp) :
  sprite(),
  sprite_name(),
  bouncing(false),
  breaking(false),
  ...
  original_y(-1)
{
  ...
  std::string sf;
  if(!lisp.get("sprite", sf) || sf.empty() || !PHYSFS_exists(sf.c_str())) {
    sf = get_default_sprite_name();
  }
